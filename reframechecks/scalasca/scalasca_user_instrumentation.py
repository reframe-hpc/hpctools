...
class SphExa_Scalasca_Profiling_User_Check(rfm.RegressionTest, hooks.setup_pe,
    ...
    def __init__(self):
        ...
        self.postrun_cmds += [
            ...
            f'cube_remap2 -d -o summary.cubex */profile.cubex',
    ...
    def set_instrumentation(self):
        ...
        sed1 = 's@#include "sphexa.hpp"@#include "sphexa.hpp"\\n#include "scorep/SCOREP_User.h"@'
        sed2 = 's@initAndGetRankId();@initAndGetRankId();\\n\\tSCOREP_RECORDING_OFF()@'
        sed0 = '92s@.*@\\tSCOREP_RECORDING_ON()\\n{SCOREP_USER_REGION("domain.sync", SCOREP_USER_REGION_TYPE_COMMON)\\n&@'  # must be sed0 (line numbers change)
        sed3 = 's@timer.step("domain::sync")@}\\nSCOREP_RECORDING_OFF()\\n\\ttimer.step("domain::sync")@'
        self.prebuild_cmds += [
            f"sed -i '{sed0}' {srcfile}",
            f"sed -i '{sed1}' {srcfile}",
            f"sed -i '{sed2}' {srcfile}",
            f"sed -i '{sed3}' {srcfile}",
        ]
    # }}}
