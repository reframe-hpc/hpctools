...
# jg (CSCS): added file output (06/2020)
...
# TXTBLK='\[\e[0;30m\]' # Black - Regular = 90
# TXTRED='\[\e[0;31m\]' # Red = 91
# TXTGRN='\[\e[0;32m\]' # Green = 92
# TXTYLW='\[\e[0;33m\]' # Yellow = 93
# TXTBLU='\[\e[0;34m\]' # Blue = 94
# TXTPUR='\[\e[0;35m\]' # Purple = magenta = 95
# TXTCYN='\[\e[0;36m\]' # Cyan = 96
# TXTWHT='\[\e[0;37m\]' # White = 97
...
def horiz_rows(fjg, labels, data, normal_dat, args, colors, doprint=True):
    ...
    for i in range(len(labels)):
        ...
        for j in range(len(values)):
            ...
            # print y axis value (mpi_tasks):
            if doprint and not args['vertical']:
                print(label, end="")
                print(label, end="", file=fjg)
                # fjg.write(label)

            # return (values[j], int(num_blocks[j]), val_min, color, label, tail, not doprint and not args['vertical'])
            yield (values[j], int(num_blocks[j]), val_min, color, label, tail, not doprint and not args['vertical'])
            # print(f'debug:-- {values[j]} {num_blocks[j]} {val_min} {color} {label} {tail} {doprint}--\n')
            # resultjg.append(values[j], int(num_blocks[j]), val_min, color, label, tail, not doprint and not args['vertical'])
...
def print_row(fjg, value, num_blocks, val_min, color, label=False, tail=False, doprint=False):
    ...
    fjg.write('\033[0m')  # no color

    if value == 0.0:
        sys.stdout.write('\033[90m')  # dark gray
        fjg.write('\033[90m')  # dark gray

    if doprint:
        print(label, tail, " ", end="")
        print(label, tail, " ", file=fjg)
        # fjg.write('%s %s' % (label, tail))
    ...
    if (num_blocks < 1 and (value > val_min or value > 0)) or (doprint and value == 0.0):
        ...
        if color:
            ...
            fjg.write('\033[{color}m'.format(color=color)) # Start to write colorized.
            fjg.write(f'\033[{color}m')  # Start to write colorized.
    ...
    if color:
        ...
        fjg.write('\033[0m')  # Back to original.
...
def stacked_graph(labels, data, normal_data, len_categories, args, colors):
    ...
    for i in range(len(labels)):
        ...
        for j in range(len(values)):
            print_row(fjg, values[j], int(num_blocks[j]), val_min, colors[j])
...
def chart(colors, data, args, labels):
    ...
    if len_categories > 1:
        ...
        if args['different_scale']:
            for i in range(len_categories):
                ...
                for row in horiz_rows(fjg, labels, cat_data, normal_cat_data,
                                      args, [colors[i]]):
                    # Print the row
    ...
    if not args['stacked']:
        ...
        sys.stdout.write('\033[0m')  # no color
        fjg.write('\033[0m')  # no color
        for row in horiz_rows(fjg, labels, data, normal_dat, args, colors, not args.get('label_before')):
            if not args['vertical']:
                print_row(fjg, *row)
            else:
                vertic = vertically(*row, args=args)
...
def check_data(labels, data, args):
    ...
    if args['color'] is not None:
        if len(args['color']) != len_categories:
            ...
            print(f">> {len(args['color'])} != {len_categories}")
            sys.exit(1)
...
def print_categories(categories, colors):
    ...
    for i in range(len(categories)):
        if colors:
            # print(f'i={i} len={len(categories)} colors_i={colors[i]}')
    ...
    print('\n\n')
    # ------------------------- cscs:
    fjg=open('termgraph.rpt', "w")
    # fjg.write(' seconds=f(mpi_tasks):')
    for i in range(len(categories)):
        if colors:
            fjg.write('\n')
            fjg.write('\033[{color_i}m'.format(color_i=colors[i])) # Start to write colorized.
            fjg.write(f'\033[{colors[i]}m')  # Start to write colorized.

        fjg.write(TICK)
        fjg.write(categories[i])
        if colors:
            fjg.write('\033[0m')  # Back to original.
